...
from db import (
    ...
    save_url_summary, get_url_summaries, delete_url_summary, save_uploaded_file
)
...
def show_pdf_quiz_page():
    ...
    if pdf_file and st.button("Generate PDF Quiz"):
        try:
            save_uploaded_file(pdf_file.name, pdf_file.getvalue(), "pdf", context="PDF Quiz")
        except Exception:
            pass
    ...
    with tab_pdf:
        ...
        if pdf_file:
            ...
            if st.button("📚 Summarize PDF", type="primary", use_container_width=True, key="btn_sum_pdf"):
                try:
                    try:
                        save_uploaded_file(pdf_file.name, pdf_file.getvalue(), "pdf", context="PDF Summarizer")
                    except Exception:
                        pass
                    reader = PyPDF2.PdfReader(pdf_file)
