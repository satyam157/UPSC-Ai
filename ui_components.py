...
from db import (
    validate_user, user_exists, get_credentials,
    create_user_session, validate_session_token, invalidate_user_session
)
...
def login_page():
    ...
    <div class="login-card">
        ...
        <div class="login-title">UPSC AI SYSTEM</div>
        <div class="login-subtitle">Multi-Tenant Civil Services Preparation Portal</div>
    </div>
    ...
    with col_m:
        with st.form("login_form", clear_on_submit=False):
            ...
            submit = st.form_submit_button("🚀 Sign In to Workspace", use_container_width=True, type="primary")
            
            if submit:
                if not username or not password:
                    st.error("⚠️ Please enter both username and password.")
                elif validate_user(username, password):
                    # Create long-lived session token (valid for 30 days to avoid unwanted timeouts)
                    token = create_user_session(username, days=30)
                    st.session_state["logged_in"] = True
                    st.session_state["username"] = username
                    st.session_state["session_token"] = token
                    
                    # Store session token in URL params for secure persistence (never trust plain username)
